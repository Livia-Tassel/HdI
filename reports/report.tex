...
\begin{abstract}
本研究基于赛题提供的 5 类数据集，对 2000--2023 年全球 203 个国家和地区的疾病谱演变、风险因素归因与卫生资源配置效率开展了数据优先的系统分析，并对中国近 20 年卫生数据进行了省级补充刻画。研究首先将全球死亡原因数据归并为“传染性疾病、非传染性疾病、伤害”三大类，构建国家--年份主面板；随后结合 HNP 与 WDI 指标，分析健康结果差异与资源条件。结果显示：2023 年全球非传染性疾病死亡占比达到 74.57\%，说明全球疾病谱转型已经非常明显；日本的预期寿命最高（约 84.0 岁），乍得最低（约 53.0 岁）；以最新截面估计，预期寿命与人均 GDP 呈显著正相关（$R^2=0.906$）。在风险归因方面，2023 年全球归因死亡规模最大的风险因素为高血压，对应归因死亡约 1083.5 万；AFRO 地区最突出的风险则转向“儿童和孕产妇营养不良”。在资源配置方面，索马里、乍得、尼日利亚等国呈现最明显的“理论需求高于实际配置”的缺口，另有 26 个国家落入“低投入--高产出”象限，显示出值得推广的效率经验。在赛题主线之外，项目还进一步围绕 Dimension 3 设计了一个英文静态 ``Optimization Lab'' 扩展模块，用于在不同优化目标与预算假设下比较卫生资源再配置方案。研究最终形成了可复现的数据处理程序、静态 API 输出、图表、英文仪表盘与 AI 协作记录，为后续在和鲸平台复现与展示提供了完整基础。
\end{abstract}
...
需要说明的是，仓库中保留了 \texttt{data/raw/external/*} 目录作为扩展占位，但这些目录当前仅含占位文件，不构成本次主线分析的实际输入。

\subsection{清洗与标准化}
真实 provided 数据与通用公开数据库脚手架之间存在明显差异，因此本项目围绕原始表真实字段做了三项关键重构：
\begin{enumerate}[label=\arabic*)]
...
赛题提供的是“风险因素归因死亡数据”，而不是暴露率与 RR 参数，因此本研究采用“归因死亡贡献占比”作为主口径，并在未来情景部分对 2023 年最重要的三类风险因素建立线性趋势基线，再构造“基准、温和干预、强化干预”三种情景。

\subsection{Dimension 3：资源缺口与再分配}
Dimension 3 采用透明的指数评估与约束优化方案：把医生密度、床位密度、卫生支出等作为投入，把寿命和死亡率等作为产出，把传染性疾病占比与儿童死亡指标作为需求代理，再在总预算约束下给出需求加权的资源重分配结果。在此基础上，项目进一步围绕同一分析链路设计了展示层扩展模块 Optimization Lab，用于对比不同优化目标与预算假设下的再分配情景。
...
\subsection{再分配模拟与 Optimization Lab}
本项目在总预算受约束、单国人均卫生支出变动受上下界限制的前提下，构建了需求加权的对数收益优化模型。基础结果显示，阿富汗、毛里塔尼亚、马达加斯加、马里、莫桑比克等高需求国家获得更高的优先度，而一些需求相对较低但当前支出较高的国家应更多承担再平衡空间。

进一步地，我们没有把资源优化停留在单一结果表，而是围绕同一套国家面板设计了一个面向展示与比较的 Optimization Lab。该扩展模块以“当前配置、优化方案、主要受益方、主要让渡方”为核心展示逻辑，把赛题第 3 维中的资源再分配结果转换为更适合评审演示与决策比较的界面。
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \texttt{ai\_agent/submission\_record.md}：面向比赛提交的协作记录摘要；
    \item \texttt{ai\_agent/log\_index.json}：日志与最终交付关系索引；
    ...
    \item \texttt{dashboard/}：英文静态仪表盘与 Optimization Lab 展示界面。
\end{itemize}

其中，\texttt{interaction\_logs/*.json} 保存的是阶段性摘要证据，而不是逐字逐轮的完整聊天全文；若某些早期日志涉及 GHRI、DEA 或其他探索性方案，应以当前报告、结果文件和 \texttt{log\_index.json} 的标注为准。
...
除完成赛题主线三维分析外，本项目的一个额外贡献是把 Dimension 3 的单次优化结果扩展为可切换目标、可切换预算、可直接演示的 Optimization Lab，使“资源重分配”从静态表格提升为可比较的决策实验界面。
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \texttt{api\_output/metadata/ghri.json} 等兼容性文件仅用于避免旧接口或旧展示逻辑报错，不代表这些内容已作为本次主线分析结果提交。
\end{itemize}
