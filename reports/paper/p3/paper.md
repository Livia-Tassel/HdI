...
全球国家情景下，$x_i$ 表示国家 $i$ 的人均卫生支出，$y_i = I_i^{\text{output}}$ 表示由预期寿命、婴儿死亡率（反向）和 5 岁以下死亡率（反向）构成的综合健康产出指数。中国省级情景下，$x_i$ 表示省份 $i$ 的人均卫生支出，$y_i$ 为由预期寿命和婴儿死亡率（反向）构成的省级健康产出指数；中国部分仍沿用单组参数 $(a, b)$。

**全球国家按四象限分组拟合**：考虑到 Q1–Q4 四类国家在投入-产出关系上存在明显结构性差异，本文为每个象限 $k \in \{\text{Q1}, \text{Q2}, \text{Q3}, \text{Q4}\}$ 单独拟合一组参数 $(a_k, b_k)$，参数由各象限截面数据最小二乘拟合得到，用于刻画"新增投入带来递减边际产出"的象限特定经验关系。

![图3-6 人均卫生支出与健康产出的对数生产函数拟合](../assets/p3_06_production_fit.png)

图3-6显示，全球国家按四象限分组后，各象限的拟合曲线在斜率和截距上均存在显著差异：Q4（低投入-低产出）的斜率参数最大（$a_{\text{Q4}}=0.4086$），说明该类国家每单位边际投入的产出增量最高；Q3（高投入-低产出）的斜率最小（$a_{\text{Q3}}=0.1446$），反映其在已有较高支出水平下的边际回报递减。各象限拟合参数汇总如下：
...
中国省级样本的拟合度较弱（$R^2 = 0.177$），这意味着在省际层面，财政投入之外的人口结构、基层服务能力和治理水平等因素同样重要。

### 总产出最大化模型（效率优先）

"最大化健康产出"通过广义注水模型求解。对全球国家情景，各国 $i$ 按所属象限 $k(i)$ 使用对应的生产函数参数 $(a_{k(i)}, b_{k(i)})$：

$$\begin{aligned} \max_{\mathbf{x}} \quad & \sum_{i=1}^{n} f_{k(i)}(x_i) = \sum_{i=1}^{n} \left[a_{k(i)} \cdot \ln(x_i + 1) + b_{k(i)}\right] \\ \text{s.t.} \quad & \sum_{i=1}^{n} x_i \leq B \\ & x_i^{\min} \leq x_i \leq x_i^{\max}, \quad i = 1, \ldots, n \end{aligned}$$
...
目标函数仍为可分离凹函数，最优解具有**广义注水（Generalized Water-Filling）结构**：各象限参数不同时，最优分配满足

$$x_i^* = \operatorname{clip}\!\left(\frac{a_{k(i)}}{\mu} - 1,\; x_i^{\min},\; x_i^{\max}\right), \quad \text{通过二分法搜索} \; \mu > 0 \; \text{使得} \; \sum_i x_i^* = B$$

### Rawlsian 最小值最大化模型（公平优先）

"最小化健康不平等"通过 Rawlsian 最小值最大化模型求解，非线性模型采用 SLSQP 求解器：

$$\begin{aligned} \max_{\mathbf{x}, \tau} \quad & \tau \\ \text{s.t.} \quad & f_{k(i)}(x_i) \geq \tau, \quad i = 1, \ldots, n \\ & \sum_{i=1}^{n} x_i \leq B \\ & x_i^{\min} \leq x_i \leq x_i^{\max} \end{aligned}$$
